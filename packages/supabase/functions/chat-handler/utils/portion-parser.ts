...
export interface NormalizedPortion extends ParsedPortion {
 ...
 */
export function parsePortion(text: string): ParsedPortion {
    ...
    return { amount: 1, unit: workText || 'piece', originalText: original }
}
 ...
 * Returns null if conversion is not possible
 */
export function convertToMl(amount: number, unit: string): number | null {
    const normalized = normalizeUnit(unit)
 ...
 */
export function formatBatchSize(grams: number, ml: number): string {
