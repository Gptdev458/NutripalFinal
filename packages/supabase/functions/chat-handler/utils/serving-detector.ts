...
export interface ServingDetectionResult {
    isSingleServing: boolean
    suggestedServings: number
    reasoning: string
    totalSizeGrams: number
 ...
 */
export function detectServingType(
    ingredients: Ingredient[],
    recipeName?: string,
    recipeText?: string
): ServingDetectionResult {
    ...
    const suggestedServings = isSingleServing
        ? 1
        : estimateServings(totalGrams, totalMl)
 ...
 */
function estimateServings(grams: number, ml: number): number {
 ...
 */
export function generateServingsPrompt(result: ServingDetectionResult): string {
 ...
 */
export function parseServingsResponse(response: string): {
    confirmed: boolean
    servings?: number
} {
    ...
    if (['yes', 'yeah', 'yep', 'correct', 'that\'s right', 'looks good', 'ok', 'okay', '1', 'one'].some(phrase => lower === phrase || lower.startsWith(phrase))) {
    ...
    for (const [text, num] of Object.entries(textNumbers)) {
        if (lower.includes(text)) {
            return { confirmed: false, servings: num }
        }
    }
