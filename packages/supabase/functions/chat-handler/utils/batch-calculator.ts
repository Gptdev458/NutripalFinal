...
export interface BatchSizeResult {
    ...
    unconvertedIngredients: string[]
    confidence: 'high' | 'medium' | 'low'
}

// Average weights for common countable ingredients (in grams)
const COUNTABLE_WEIGHTS: Record<string, number> = {
    'egg': 50,
    'eggs': 50,
    'egg white': 33,
    'egg yolk': 17,
 ...
 */
export function calculateBatchSize(ingredients: Ingredient[]): BatchSizeResult {
    let totalGrams = 0
    let totalMl = 0
    const breakdown: BatchSizeResult['ingredientBreakdown'] = []
 ...
 */
function findCountableWeight(ingredientName: string, unit: string): number | null {
 ...
 */
export function generateBatchConfirmationPrompt(result: BatchSizeResult): string {
 ...
 */
export function parseBatchSizeResponse(response: string): {
    confirmed: boolean
    ...
    if (['yes', 'yeah', 'yep', 'correct', 'that\'s right', 'looks good', 'ok', 'okay'].some(phrase => lower === phrase || lower.startsWith(phrase))) {
