...
import { FoodLogEntry, UserGoal, NutritionData, InsightResult, Agent, AgentContext } from '../../_shared/types.ts'
import { getStartAndEndOfDay, getDateRange } from '../../_shared/utils.ts'

export class InsightAgent implements Agent<void, InsightResult> {
  name = 'insight'
...
export async function generateInsights(userId: string): Promise<InsightResult> {
  ...
  return agent.execute(undefined, { userId, supabase: createAdminClient() } as any)
}
